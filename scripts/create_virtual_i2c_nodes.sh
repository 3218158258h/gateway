...
create_real_links() {
  echo "[INFO] Creating $COUNT I2C gateway node link(s) from real /dev/i2c-* under $BASE_DIR"
  for ((i=0; i<COUNT; i++)); do
    gw_node="$BASE_DIR/i2c-gw${i}"
    target_dev="${real_devs[$i]}"
    rm -f "$gw_node"
    ln -s "$target_dev" "$gw_node"
    printf "%s\t%s\t%s\t%s\t%s\n" "$i" "i2c" "$gw_node" "$target_dev" "real" >> "$MAP_FILE"
    echo "[OK] i2c-node[$i] gw=$gw_node -> $target_dev mode=real"
  done
  echo
...
create_pseudo_pairs() {
  ...
  echo "[WARN] Falling back to pseudo I2C PTY pairs."
  echo "[WARN] pseudo mode cannot pass I2C ioctl (I2C_TENBIT/I2C_SLAVE); only byte-stream integration test."
  ...
  for ((i=0; i<COUNT; i++)); do
    ...
    printf "%s\t%s\t%s\t%s\t%s\t%s\n" "$i" "i2c" "$gw_node" "$sim_node" "$pid" "pseudo" >> "$MAP_FILE"
    echo "[OK] i2c-pair[$i] gw=$gw_node <-> sim=$sim_node pid=$pid mode=pseudo"
  done
...
if [[ "$MODE" == "real" ]]; then
  if [[ "${#real_devs[@]}" -lt "$COUNT" ]]; then
    echo "[ERROR] real mode requested, but only found ${#real_devs[@]} /dev/i2c-* device(s), count=$COUNT" >&2
    exit 1
  fi
  create_real_links
  exit 0
