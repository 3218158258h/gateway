...
echo "[INFO] Creating $COUNT virtual SPI-like pair(s) under $BASE_DIR"
echo "[INFO] note: these are byte-stream PTY nodes for integration simulation, not real kernel spidev devices."
...
for ((i=0; i<COUNT; i++)); do
  ...
  printf "%s\t%s\t%s\t%s\t%s\n" "$i" "spi" "$gw_node" "$sim_node" "$pid" >> "$MAP_FILE"
  echo "[OK] spi-pair[$i] gw=$gw_node <-> sim=$sim_node pid=$pid"
done
